...
#Extract COI reads
bbmap.sh build=1 t=${NSLOTS} in=../filtered_reads/${FILE}_R1_clean.fastq.gz in2=../filtered_reads/${FILE}_R2_clean.fastq.gz ambiguous=best pairedonly=t outm=COI/${FILE}_COI_#.fastq
cd COI
...
blastn -query ${FILE}_COI.fasta -db /scratch/nmnh_mdbc/breusingc/databases/BOLD_COI.27-Feb-2026 -out blast.${FILE}.COI.txt -evalue 1e-10 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt 6
cat blast.${FILE}.COI.txt | sort -k1,1 -k12,12nr -k11,11n | sort -u -k1,1 > blast.${FILE}.COI.topHit.txt
perl /home/breusingc/scripts/reformat_table.pl blast.${FILE}.COI.topHit.txt /scratch/nmnh_mdbc/breusingc/databases/BOLD_COI.2026_02_27.txt blast.${FILE}.COI.topHit.full.txt
mv blast.${FILE}.COI.topHit.full.txt blast.${FILE}.COI.topHit.txt
rm blast.${FILE}.COI.txt
blastn -query ${FILE}_COI.fasta -db /scratch/dbs/blast/v5/mito/mito -out blast.${FILE}.mito.txt -evalue 1e-10 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt "6 std stitle staxids"
...
#Extract 18S reads
bbmap.sh build=2 t=${NSLOTS} in=../filtered_reads/${FILE}_R1_clean.fastq.gz in2=../filtered_reads/${FILE}_R2_clean.fastq.gz ambiguous=best pairedonly=t outm=18S/${FILE}_18S_#.fastq
cd 18S
...
blastn -query ${FILE}_18S.fasta -db /scratch/nmnh_mdbc/breusingc/databases/SILVA_138.1_SSURef_NR99 -out blast.${FILE}.18S.txt -evalue 1e-10 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt "6 std stitle"
cat blast.${FILE}.18S.txt | sort -k1,1 -k12,12nr -k11,11n | sort -u -k1,1 > blast.${FILE}.18S.topHit.txt
rm blast.${FILE}.18S.txt
blastn -query ${FILE}_18S.fasta -db /scratch/dbs/blast/v5/core_nt/core_nt -out blast.${FILE}.18Snt.txt -evalue 1e-10 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt "6 std stitle staxids"
...
#Extract 28S reads
bbmap.sh build=3 t=${NSLOTS} in=../filtered_reads/${FILE}_R1_clean.fastq.gz in2=../filtered_reads/${FILE}_R2_clean.fastq.gz ambiguous=best pairedonly=t outm=28S/${FILE}_28S_#.fastq
cd 28S
...
blastn -query ${FILE}_28S.fasta -db /scratch/nmnh_mdbc/breusingc/databases/SILVA_138.1_LSURef_NR99 -out blast.${FILE}.28S.txt -evalue 1e-10 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt "6 std stitle"
cat blast.${FILE}.28S.txt | sort -k1,1 -k12,12nr -k11,11n | sort -u -k1,1 > blast.${FILE}.28S.topHit.txt
rm blast.${FILE}.28S.txt
blastn -query ${FILE}_28S.fasta -db /scratch/dbs/blast/v5/core_nt/core_nt -out blast.${FILE}.28Snt.txt -evalue 1e-10 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt "6 std stitle staxids"
