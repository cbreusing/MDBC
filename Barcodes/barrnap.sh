...
blastn -query ${file}_28S_rRNA.fasta -db /scratch/dbs/blast/v5/nt -out blast.${file}.28Snt.txt -evalue 1e-20 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt "6 std stitle staxids"
cat blast.${file}.28Snt.txt | sort -k1,1 -k3,3nr -k12,12nr -k11,11n | sort -u -k1,1 > blast.${file}.28Snt.topHit.txt
...
blastn -query ${file}_28S_rRNA.fasta -db /scratch/nmnh_mdbc/breusingc/databases/SILVA_138.1_LSURef_NR99 -out blast.${file}.28S.txt -evalue 1e-20 -max_target_seqs 50 -outfmt "6 std stitle"
cat blast.${file}.28S.txt | sort -k3,3nr -k1,1 -k12,12nr -k11,11n | sort -u -k1,1 > blast.${file}.28S.topHit.txt
rm blast.${file}.28S.txt
blastn -query ${file}_18S_rRNA.fasta -db /scratch/dbs/blast/v5/nt -out blast.${file}.18Snt.txt -evalue 1e-20 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt "6 std stitle staxids"
cat blast.${file}.18Snt.txt | sort -k3,3nr -k1,1 -k12,12nr -k11,11n | sort -u -k1,1 > blast.${file}.18Snt.topHit.txt
...
blastn -query ${file}_18S_rRNA.fasta -db /scratch/nmnh_mdbc/breusingc/databases/SILVA_138.1_SSURef_NR99 -out blast.${file}.18S.txt -evalue 1e-20 -max_target_seqs 50 -outfmt "6 std stitle"
