...
file=$(sed -n "${SGE_TASK_ID}p" files.txt | perl -anle 'print $F[0]')
taxon=$(sed -n "${SGE_TASK_ID}p"  files.txt | perl -anle 'print $F[1]')
...
barrnap --kingdom euk --threads ${NSLOTS} --outseq ${file}_18S-28S_rRNA.fasta < /scratch/nmnh_mdbc/breusingc/genohub_6762505/metagenomes/${file}_metagenome.fasta
conda deactivate
...
blastn -query ${file}_28S_rRNA.fasta -db /scratch/dbs/blast/v5/core_nt/core_nt -out blast.${file}.28Snt.txt -evalue 1e-10 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt "6 std stitle staxids"
...
blastn -query ${file}_18S_rRNA.fasta -db /scratch/dbs/blast/v5/core_nt/core_nt -out blast.${file}.18Snt.txt -evalue 1e-10 -num_threads ${NSLOTS} -max_target_seqs 50 -outfmt "6 std stitle staxids"
